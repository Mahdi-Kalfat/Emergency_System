...
const Header = ({ sidebarToggle, setSidebarToggle }) => {
  ...
  const [todaysRendezvous, setTodaysRendezvous] = useState([]);
  const [newNotification, setNewNotification] = useState(false); // State for new notification
  const [fetchTrigger, setFetchTrigger] = useState(false); // Trigger for fetching data
  ...
  useEffect(() => {
    const fetchTodaysRendezvous = async () => {
      try {
        const response = await axios.get("http://localhost:3001/rendezvous/display");
        const today = new Date().toISOString().split("T")[0];
        const filteredRendezvous = response.data.filter((rdv) =>
          rdv.date.startsWith(today)
        );
        setTodaysRendezvous(filteredRendezvous);
        setNewNotification(filteredRendezvous.length > todaysRendezvous.length);
      } catch (error) {
    ...
    if (fetchTrigger) {
      fetchTodaysRendezvous();
      setFetchTrigger(false); // Reset the trigger after fetching
    }
  }, [fetchTrigger]);

  const triggerFetch = () => setFetchTrigger(true); // Function to trigger fetching

  // Example: Call triggerFetch when a new appointment is added
  useEffect(() => {
    const interval = setInterval(() => {
      triggerFetch(); // Trigger fetch periodically (e.g., every 10 seconds)
    }, 10000);

    return () => clearInterval(interval); // Cleanup interval on component unmount
  }, []);

  const dismissNotification = () => setNewNotification(false);
  ...
  const joinMeeting = async () => {
    ...
    // Fetch the room name from the backend
    let roomName;
    try {
      const response = await axios.get("http://localhost:3001/meet/create-room");
    ...
    const options = {
      roomName, // Use the room name fetched from the backend
  ...
  return (
    <header className="sticky top-0 flex w-full bg-white border-gray-200 z-40 dark:border-gray-800 dark:bg-gray-900 lg:border-b">
                ...
                <div className="absolute -right-[240px] mt-[17px] flex h-[480px] w-[350px] flex-col rounded-2xl border border-gray-200 bg-white p-3 shadow-theme-lg dark:border-gray-800 dark:bg-gray-dark sm:w-[361px] lg:right-0">
                  ...
                  <ul className="flex flex-col h-auto overflow-y-auto custom-scrollbar">
                    {todaysRendezvous.length > 0 ? (
                      todaysRendezvous.map((rdv, index) => (
                        <li key={index}>
                          <div className="flex flex-col gap-3 rounded-lg border-b border-gray-100 p-3 px-4.5 py-3 hover:bg-gray-100 dark:border-gray-800 dark:hover:bg-white/5">
                            <div>
                            ...
                            <button
                              ...
                              onClick={() => joinMeeting()} // Call joinMeeting when clicked
                            >
                              Join Meeting
                            </button>
                          </div>
                      ...
                      <li>
                        <p className="text-gray-500 text-center">No rendezvous for today.</p>
                      </li>
                    )}
                  </ul>
                </div>
      ...
      {newNotification && (
        <div className="fixed top-5 right-5 z-50 flex items-center gap-4 p-4 bg-orange-100 border border-orange-400 rounded-lg shadow-lg dark:bg-orange-900 dark:border-orange-700">
          <svg
          ...
          <span className="text-sm font-medium text-orange-700 dark:text-orange-300">
            New notification: There is a new rendezvous!
          </span>
          <button
            onClick={dismissNotification}
            className="text-orange-500 hover:text-orange-700 dark:text-orange-300 dark:hover:text-orange-500"
          >
            <svg
              className="w-5 h-5"
              xmlns="http://www.w3.org/2000/svg"

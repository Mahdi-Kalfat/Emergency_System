...
const EditPersonalForm = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    conge: "",
    nbr_conge: "",
    password: "", // Keep the password in state but do not allow editing
  });
    ...
    const fetchUserData = async () => {
      try {
        ...
        setFormData({
          ...
          role: userData.role,
          password: userData.password, // Set the existing password
          age: userData.age,
  ...
  const handleSubmit = async (e) => {
    ...
    const userId = formData.id; // Get the ID from the hidden input field

    console.log("Form Data:", personalData);
    console.log("ID:", userId); // Debug: Log the ID from the hidden input field
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:3001/users/editUser/${userId}`, // Use the ID from the hidden input field
        personalData,
        {
          headers: {
